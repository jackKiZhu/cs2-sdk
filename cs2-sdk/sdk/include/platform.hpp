...
#pragma warning(disable : 4748)  // warning C4748: /GS can not protect parameters and local variables from local buffer overrun because
...
#pragma warning(disable : 4530)  // warning C4530: C++ exception handler used, but unwind semantics are not enabled. Specify /EHsc (disabled
                                 // due to std headers having exception syntax)
#endif
...
static FORCEINLINE float fsel(float fComparand, float fValGE, float fLT) { return fComparand >= 0 ? fValGE : fLT; }
static FORCEINLINE double fsel(double fComparand, double fValGE, double fLT) { return fComparand >= 0 ? fValGE : fLT; }
...
#else
FORCEINLINE unsigned long LoadLittleDWord(const unsigned long *base, unsigned int dwordIndex) { return LittleDWord(base[dwordIndex]); }
...
#define GET_OUTER(OuterType, OuterMember) ((OuterType *)((uint8 *)this - offsetof(OuterType, OuterMember)))
