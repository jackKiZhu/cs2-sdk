...
enum { COLOR_R = 0, COLOR_G = 1, COLOR_B = 2, COLOR_A = 3 };
...
struct Color_t {
    ...
    [[nodiscard]] constexpr ImU32 GetU32(const float flAlphaMultiplier = 1.0f) const { return IM_COL32(r, g, b, a * flAlphaMultiplier); }
    ...
    static Color_t FromBase3(const float arrBase[3]) { return {arrBase[0], arrBase[1], arrBase[2]}; }
    ...
    /// @returns : color created from float[3] array (in: [0.0 .. 1.0], out: [0 .. 255])
    static Color_t FromBase4(const float arrBase[4]) { return {arrBase[COLOR_R], arrBase[COLOR_G], arrBase[COLOR_B], arrBase[COLOR_A]}; }
    ...
    void ToHSB(float (&arrHSB)[3]) const {
        ...
        if (arrBase[COLOR_G] < arrBase[COLOR_B]) {
            std::swap(arrBase[COLOR_G], arrBase[COLOR_B]);
            flKernel = -1.0f;
        }
        if (arrBase[COLOR_R] < arrBase[COLOR_G]) {
            std::swap(arrBase[COLOR_R], arrBase[COLOR_G]);
            flKernel = -2.0f / 6.0f - flKernel;
        }

        const float flChroma = arrBase[COLOR_R] - std::min(arrBase[COLOR_G], arrBase[COLOR_B]);
        arrHSB[COLOR_R] =
            std::fabsf(flKernel + (arrBase[COLOR_G] - arrBase[COLOR_B]) / (6.0f * flChroma + std::numeric_limits<float>::epsilon()));
