...
class CUtlFixedMemory {
   ...
   public:
    ...
    class Iterator_t {
       public:
        Iterator_t(BlockHeader_t* pBlockHeader, const std::intptr_t nIndex) : pBlockHeader(pBlockHeader), nIndex(nIndex) {}

        bool operator==(const Iterator_t it) const { return pBlockHeader == it.pBlockHeader && nIndex == it.nIndex; }
    ...
    [[nodiscard]] Iterator_t First() const { return (pBlocks != nullptr ? Iterator_t(pBlocks, InvalidIndex()) : InvalidIterator()); }
    ...
    [[nodiscard]] Iterator_t Next(const Iterator_t& it) const {
        ...
        if (it.nIndex + 1 < pHeader->nBlockSize) return Iterator_t(pHeader, it.nIndex + 1);
    ...
    [[nodiscard]] bool IsIndexAfter(std::intptr_t nIndex, const Iterator_t& it) const {
        ...
        if (IsInBlock(nIndex, it.pBlockHeader)) return nIndex > GetIndex(it);

        for (BlockHeader_t* pBlockHeader = it.pBlockHeader->pNext; pBlockHeader != nullptr; pBlockHeader = pBlockHeader->pNext) {
            if (IsInBlock(nIndex, pBlockHeader)) return true;
        }
    ...
    [[nodiscard]] Iterator_t InvalidIterator() const { return Iterator_t(nullptr, InvalidIndex()); }

    [[nodiscard]] bool IsValidIndex(const std::intptr_t nIndex) const { return nIndex != InvalidIndex(); }

    [[nodiscard]] static std::intptr_t InvalidIndex() { return 0; }

    [[nodiscard]] int Count() const { return nAllocationCount; }

    void EnsureCapacity(const int nCapacity) { Grow(nCapacity - Count()); }
    ...
    void Grow(const int nCount = 1) {
        ...
        if (nBlockSize < nCount) nBlockSize *= (nCount + nBlockSize - 1) / nBlockSize;

        nAllocationCount += nBlockSize;

        BlockHeader_t* pNewBlockHeader =
            static_cast<BlockHeader_t*>(IMemAlloc::Get().Alloc(sizeof(BlockHeader_t) + nBlockSize * sizeof(T)));
        ...
        if (pBlocks == nullptr)
            ...
            while (pBlockHeader->pNext != nullptr) pBlockHeader = pBlockHeader->pNext;
   ...
   protected:
    ...
    [[nodiscard]] const T* HeaderToBlock(const BlockHeader_t* pHeader) const { return reinterpret_cast<const T*>(pHeader + 1); }

    [[nodiscard]] const BlockHeader_t* BlockToHeader(const T* pBlock) const { return reinterpret_cast<const BlockHeader_t*>(pBlock) - 1; }
