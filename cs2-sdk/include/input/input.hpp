...
class CInputHandler {
   public:
    ...
    bool IsPressed(uint64_t button) const { return IsScrolled(button) || ((buttonsHeld & button) && !(prevButtonsHeld & button)); }
    bool IsReleased(uint64_t button) const { return IsScrolled(button) || (!(buttonsHeld & button) && (prevButtonsHeld & button)); }
